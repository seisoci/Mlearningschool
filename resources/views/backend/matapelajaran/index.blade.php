...
@section('content')
    <div class="container-fluid">
        <h4 class="c-grey-900 mT-10 mB-30">Matapelajaran Tables</h4>
        <div class="row">
            <div class="col-md-12">
                <div class="bgc-white bd bdrs-3 p-20 mB-20">
                    <div class="d-flex justify-content-between mb-3">
                        <h4 class="c-grey-900">Matapelajaran Data Table</h4>
    ...
    <div class="modal fade" id="modalAdd" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            ...
            <form id="formStore" action="{{ route('matapelajaran.store') }}">
            <div class="modal-body">
                @csrf
                <div class="form-group">
                    <label for="exampleInput">Nama Matapelajaran</label>
                    <input type="text" class="form-control" name="nama_matapelajaran" placeholder="Nama Matapelajaran">
                </div>
            </div>
...
<script>
   $(document).ready(function(){
        var datatable = $('.datatable').DataTable({
            ...
            scrollX: true,
            ajax: {
                url: "{{ route('matapelajaran.datatable')}}",
                type: 'GET',
                data: function (d) {
                    d.guru = $('#guru').val();
                    d.user = $('#user').val();
            ...
            order: [[0, 'asc']],
            columns: [
                { data: 'nama_matapelajaran', name: 'nama_matapelajaran' },
        ...
        $('#modalUpdate').on('show.bs.modal', function (event) {
            ...
            var nama_matapelajaran = $(event.relatedTarget).data('name');
            $(this).find('#formUpdate').attr('action', '{{ route("matapelajaran.index") }}/'+ id);
        ...
        $('#modalDelete').on('show.bs.modal', function (event) {
            var id = $(event.relatedTarget).data('id');
            $(this).find('.modal-body').find('a[name="id"]').attr('href', '{{ route("matapelajaran.index") }}/'+ id);
        });
